...
from modules.nlp import NLPDataset, NLPHierarchyCorrector, NLPErrorPredictor
...
def run(cfg: ModelConfig,
    ...
    # Inference
        ...
        if return_levels:
            ...
            if not error.empty:
                print(error)
                # error.to_csv("data/suspect_errors.csv", index=True)
            else:
                ...
                # top_10_suspects.to_csv("data/suspect_errors.csv", index=True)
            
        print("\n Les résultats sont enregistrés dans le répertoire : data/.")
...
result = run(
    ...
    model_save_name = "model_only_label_0922_local.pt",
    model_read_name = "model_only_label_0922_local.pt",
    return_levels=False
)